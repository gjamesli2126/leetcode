...
public:
    vector<pair<int,int>> ind_vals;
    SparseVector(vector<int> &vec1) {
        for(int i=0;i<vec1.size();i++){
            if(vec1[i]) ind_vals.emplace_back(i,vec1[i]);
        }
    }
    ...
    int dotProduct(SparseVector& svec2) {
        int p=0,q=0;
        int totprod=0;
        while(p<ind_vals.size() && q<svec2.ind_vals.size()){
            if(ind_vals[p].first==svec2.ind_vals[q].first) {
                totprod += ind_vals[p].second * svec2.ind_vals[q].second;
                p++;
                q++;
            }
            else if(ind_vals[p].first<svec2.ind_vals[q].first) p++;
            else q++;
        }
        return totprod;
    }
