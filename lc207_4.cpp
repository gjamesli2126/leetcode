...
private:
    ...
    unordered_map<int,vector<int>> this_nexts;
    bool dfs(int this_course,vector<state>& statVec){
        if(statVec[this_course]==visiting) return false;
        else if(statVec[this_course]==visited) return true;//all done
        statVec[this_course]=visiting;
        for(int nx:this_nexts[this_course]){
            if(!dfs(nx,statVec)) return false;
        }
        statVec[this_course]=visited;
        return true;
    }
public:
    bool canFinish(int numCourses, vector<vector<int>>& prerequisites) {
        //using DFS and visited all nodes;
        //build the damn graph
        for(auto& this_preq:prerequisites) this_nexts[this_preq[1]].push_back(this_preq[0]);

        //traverse w/ DDDDDDDFs
        vector<state> statVec(numCourses,unvisited);
        for(int course=0;course<numCourses;course++){
            if(statVec[course]!=unvisited) continue;
            //only unvisited will let in
            if(!dfs(course,statVec)) return false;
        }
        return true;
    }
};
