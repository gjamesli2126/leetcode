...
public:
    Node* copyRandomList(Node* head) {
        //create table: val:new Node()
        unordered_map<Node*,Node*> org_cpy;
        //run thru nextS w/ random pointer to create table
        Node* node=head;
//        int head_val=head->val;
        while(node){
            org_cpy[node]=new Node(node->val);
            node=node->next;
        }
        //assign next & random
        node=head;
        while(node){
            org_cpy[node]->next=node->next?org_cpy[node->next]:NULL;
            org_cpy[node]->random=node->random?org_cpy[node->random]:NULL;
            node=node->next;
        }
        return org_cpy[head];
    }
