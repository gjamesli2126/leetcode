...
class Solution {
private:
    unordered_map<int,Node*> val_new;//perform sync
    unordered_map<int,vector<int>> neis;
    void bfsBuildGraph(Node* root){
        if(!root) return;
        queue<Node*> q;
        q.push(root);
        ...
        while(!q.empty()){
            ...
            val_new[node->val]=new Node(node->val);
            for(Node* nei:node->neighbors){
                neis[node->val].push_back(nei->val);
                q.push(nei);
            }
...
public:
    Node* cloneGraph(Node* root) {
        ...
        bfsBuildGraph(root);
        //now we have val_new relation, int->neis relation
        for(auto [val,node]:val_new){
            for(int nei_val:neis[val]) node->neighbors.push_back(val_new[nei_val]);
        }
        return val_new[root->val];
    }
};
