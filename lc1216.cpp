...
class Solution {
private:
    int dfsNeedK(int i,int j,const string& s, vector<vector<int>>& memo){
        if(i>=j) return 0;
        int& ans=memo[i][j];
        if(ans!=-1) return ans;
        if(s[i]==s[j]) ans= dfsNeedK(i+1,j-1,s,memo);
        else ans=1+ min(dfsNeedK(i,j-1,s,memo), dfsNeedK(i+1,j,s,memo));
        return ans;
...
public:
    bool isValidPalindrome(const string& s, int k) {
        ...
        return dfsNeedK(0,n-1,s,memo)<=k;
    }
