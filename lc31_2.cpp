...
public:
    void nextPermutation(vector<int>& nums) {
        ...
        //find the pivot point from the back
        for(int i=nums.size()-1;i>0;i--){
            if(nums[i-1]>=nums[i]) continue;
            //found the pivot: nums[i], to be swapped: nums[i-1] & nums[j]
            for(int j=nums.size()-1;j>=i;j--){
                if(nums[j]<=nums[i-1]) continue;
                //found the first bigger num(j) from the back
                swap(nums[j],nums[i-1]);
                //reverse from the pivot point to last
                reverse(nums.begin()+i,nums.end());
                return;
            }

        }
        //if is the max number Then back to smallest
