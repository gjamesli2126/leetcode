...
class Solution {
private:
    unordered_map<int,unordered_set<int>> this_prev;
    bool dfs(int thisCourse,vector<stat>& v){
        if(v[thisCourse]==stat::visiting) return false;
        else if(v[thisCourse]==stat::visited) return true;
        //unvisit
        v[thisCourse]=stat::visiting;
        for(int p:this_prev[thisCourse]){
            if(!dfs(p,v)) return false;
        }
        v[thisCourse]=stat::visited;
        return true;
    }
public:
    bool canFinish(int numCourses, vector<vector<int>>& prerequisites) {
        //topological && DFS
        for(vector<int> tp:prerequisites){
            this_prev[tp[0]].insert(tp[1]);
        }
        for(const auto& [thisCourse,pres]:this_prev) {
            vector<stat> v(numCourses,unvisted);
            if (!dfs(thisCourse,v)) return false;
        }
        return true;
    }
};
