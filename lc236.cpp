...
private:
    TreeNode* intersect;
    int dfs(TreeNode* node, const TreeNode* p,const TreeNode* q){
        if(!node) return 0;
        int left=dfs(node->left,p,q);
        int mid=(node==p || node==q);
        int right= dfs(node->right,p,q);
        if(left+mid+right>=2) intersect=node;
        return left||mid||right;
    }
public:
    TreeNode* lowestCommonAncestor(TreeNode* root, TreeNode* p, TreeNode* q) {
        if(!root) return nullptr;
        //let both a & b run the same distance if have parent ptr. But does not have now so use true-true flag
        dfs(root,p,q);
        return intersect;
    }
};
