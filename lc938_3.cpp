...
private:
    int sum;
    int low,high;
    inline bool inRange(TreeNode* node){
        return low<=node->val && node->val<=high;
    }
    void dfs(TreeNode* node){
        if(!node) return;
        if(low<node->val) dfs(node->left);
        if(inRange(node)) sum+=node->val;
        if(node->val<high) dfs(node->right);
        //so a number is in the range, it will also traverse to left tree & right tree
    }
public:
    int rangeSumBST(TreeNode* root, int low0, int high0) {
        low=low0;
        high=high0;
        sum=0;
        dfs(root);
