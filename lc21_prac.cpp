...
class Solution {
private:
    ListNode* minVal(ListNode* node1,ListNode* node2){
        if(!node1 && !node2) return nullptr;
        if(!node1) return node2;
        if(!node2) return node1;

        if(node1->val>node2->val) return node2;
        return node1;
    }
public:
    ListNode* mergeTwoLists(ListNode* list1, ListNode* list2) {
        ...
        ListNode *cur= minVal(list1,list2);
        ...
        while(cur){
            ...
            cur->next= minVal(nx1,nx2);//link first
            cur=cur->next;//then move forward
        }
        return newStart;
    }
};
