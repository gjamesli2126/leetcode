...
public:
    int depthSum(vector<NestedInteger>& nestedList) {
        //using que to use BFS
        int total=0,depth=1;
        deque<NestedInteger> q;
        for(const NestedInteger& ni0:nestedList) q.emplace_back(ni0);
        while(!q.empty()){
            size_t size=q.size();
            for(int i=0;i<size;i++){
                NestedInteger ni=q.front();
                q.pop_front();
                if(ni.isInteger()) total+=ni.getInteger()*depth;
                else{
                    for(const NestedInteger& nii:ni.getList()) q.push_back(nii);
                }
            }
            depth++;
        }
